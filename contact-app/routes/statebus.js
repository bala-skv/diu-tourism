...
router.post('/', (req, res) => {
    ...
    db.query('INSERT INTO state_buses (from_location, to_location, bus_time, travels, cost) VALUES (?, ?, ?, ?, ?)',
        [from_location, to_location, bus_time, travels, cost],
        (err, result) => {
            if (err) return res.status(500).json({ error: err.message });
            res.json({ message: 'State bus added successfully', busId: result.insertId });
        });
});
